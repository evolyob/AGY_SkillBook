# Production Coding Standards & AST Static Audit Guide

This guide integrates core architectural governance standards with automated AST static code analysis. It provides modern engineering specifications spanning architectural mindsets, standard operating procedures, and automated machine verification.
...
│  • Role: Design radar & mindfulness checklist (Engineering Heuristics) │
...
## 2. Architectural Coding Standards & Specifications

### Quick Reference & Pragmatic Boundaries

| Standard / Principle | Focus Area | Architectural Heuristic | Pragmatic Boundary |
| :--- | :--- | :--- | :--- |
| **Return Early** | Eliminate nested pyramid conditionals | Use top Guard Clauses; use `match...case` for branching | 1~2 levels of shallow `if` are valid; avoid fragmenting micro-functions |
| **Name the Meaning** | Avoid generic names (`data`) & boolean blindness | Domain entity naming; enforce keyword-only (`*`) for flags | 1~2 arg pure math or helper functions (`add(a, b)`) exempt |
| **Own the Boundary** | Isolate external dependencies & token limits | **Spec-First**: define schema before coding; keep bulk data on disk | Standard library calls (`pathlib.Path`) require no anti-corruption layer |
| **Model the State** | Eliminate invalid states & clamp lower bounds | Use `StrEnum` / Unions; defensively clamp geometry with `max(MIN, ...)` | Temporary internal script dicts do not need full class modeling |
| **Split the Decision** | Decouple pure computation from side effects | Functional core with zero I/O; shell orchestrates I/O and network | Simple sequential conversion scripts (read -> transform -> save) exempt |
| **Useful Errors** | Actionable context for machines & humans | Domain exceptions; preserve stack trace with `raise ... from err` | Small scripts can use stdlib exceptions (`ValueError`) without over-engineering |
| **Ship Small Diffs** | Atomic PRs & laser-focused changes | **Chunking**: 3~5 milestones; **Root Cause**: grep all callers | Single-file modular scripts under 300~400 lines remain healthy |

---

### Gate 1: Control Flow & Structure Gate

#### 1.1 Return Early (Keep Happy Paths Flat)
...
```
```python
# [Compliant]: Guard Clauses with early return; zero-indent happy path
...
#### 2.1 Name the Meaning (Intent-Revealing Names)
...
```
```python
# [Compliant]: Domain naming & mandatory keyword-only flag
...
#### 2.2 Own the Boundary (Isolate External I/O & Prompt Context)
...
```
```python
# [Compliant]: Spec-First strongly-typed immutable models
...
#### 3.1 Model the State (Illegal States Unrepresentable)
...
```
```python
# [Compliant]: None fallback & defensive max() lower bound clamping
...
#### 3.2 Split the Decision (Decouple Pure Logic from Side Effects)
...
```
```python
# [Compliant]: Pure calculation isolated in Functional Core (zero mocks needed)
...
#### 4.1 Useful Errors (Machine-Actionable & Human-Readable)
...
```
```python
# [Compliant]: Domain exception chaining preserving root cause
try:
    process_payment()
...
#### 4.2 Ship Small Diffs (Keep Changes Focused & Atomic)
...
```
```python
# [Compliant]: Root-cause defense consolidated in shared core function
def get_data() -> dict:
    raw = fetch()
...
### Non-Compliant Pattern (Junior Code / 7 Violations / Audit FAIL)
...
### Compliant Production Code (100% Compliant with Architectural Standards / Audit PASS)
...
* **Action**: Set upfront constraints in project rules (standard library first, explicit dependencies only, preserve existing tests).
* **Prompt Template**:
  > "This project strictly enforces modular architectural standards and minimal dependency principles. Prioritize standard library capabilities; undeclared third-party packages are prohibited."

### Step 2: Spec-First Design
