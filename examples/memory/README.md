...
```mermaid
graph TD
    Boot["Agent Session Boot"] --> LoadCore["Load Core Index: ~/.gemini/memory/core.md (< 30 lines)"]
    ...
    Evaluate -->|"Match Topic 3"| LoadT3["On-Demand Load: topics/user_preferences.md"]
    Evaluate -->|"No Specific Trigger"| FastExecute["Execute with Zero Memory Context Bloat"]
```

* **Tier 1: Core Memory Index (`core.md`)**: A lightweight router (< 30 lines) loaded at session boot. Maps domain keywords and intent triggers to specialized topic files.
...
## 2. Foundation Memory Suite (3 Universal Pillars)
...
| **`user_preferences.md`** | **User Directives & Safety Budgets** | `/learn` | **Preferences & Persistence**: Standardizes export paths (`~/Downloads`), cross-platform file encoding (`utf-8-sig`), safety limits (download thresholds, log retention), and explicit memory mutation triggers. |
...
```text
examples/memory/
├── core.md                                   # Global index & routing keywords (< 30 lines)
...
└── topics/
    ...
    └── user_preferences.md                   # Storage directives, encoding & resource budgets
```
...
Ensure your Antigravity global configuration or system constitution (`~/.gemini/config/config.json` or `GEMINI.md`) includes persistent memory indexing:
...
When adding proprietary or project-specific topics (e.g., threat intelligence feeds, CI/CD automation, cloud infrastructure):
...
2. **Register in `core.md`**:
   ...
   - Domain category
   - File path relative to `~/.gemini/memory/`
